...
class DTATask(object):
    ...
    def __init__(self,
        ...
        random.seed(42)
        drug_counts = OrderedDict(Counter(rec['drug_name'] for rec in self.data_list))
        protein_counts = OrderedDict(Counter(rec['protein_name'] for rec in self.data_list))
        split_point = int(len(drug_counts) * 0.1)
        split_point1 = int(len(protein_counts) * 0.1)
        test_part = list(drug_counts.items())[:split_point]
        train_valid_part = list(drug_counts.items())[split_point:]
        train_valid_part1 = list(protein_counts.items())[split_point1:]
        test_part = OrderedDict(test_part)
        train_valid_part = OrderedDict(train_valid_part)
        train_valid_part1 = OrderedDict(train_valid_part1)
        test_drugs = [drug for drug, count in test_part.items()]
        train_valid_drugs = [drug for drug, count in train_valid_part.items()]
        train_valid_drugs1 = [drug for drug, count in train_valid_part1.items()]

        valid_drugs = train_valid_drugs[:split_point]
        train_drugs = train_valid_drugs[split_point:]
        valid_drugs1 = train_valid_drugs1[:split_point]
        train_drugs1 = train_valid_drugs1[split_point:]

        drug_records_dict = defaultdict(list)
        self.train_set = []
        self.test_set = []
        self.valid_set = []
        for record in self.data_list:
            drug_name = record['drug_name']
            drug_records_dict[drug_name].append(record)
        for drug in train_drugs:
            if drug in drug_records_dict:
                self.train_set.extend(drug_records_dict[drug])
        for drug in valid_drugs:
            if drug in drug_records_dict:
                self.valid_set.extend(drug_records_dict[drug])
        for drug in test_drugs:
            if drug in drug_records_dict:
                self.test_set.extend(drug_records_dict[drug])

        for record in self.data_list:
            protein_name = record['protein_name']
            drug_records_dict[protein_name].append(record)
        for protein in train_drugs1:
            if protein in drug_records_dict:
                self.train_set.extend(drug_records_dict[protein])
        for protein in valid_drugs1:
            if protein in drug_records_dict:
                self.valid_set.extend(drug_records_dict[protein])
        # selected_train_drugs = [drug for drug, count in drug_counts.items() if count >= 50]
        # if len(selected_train_drugs) > 60:
        #     high_drugs = random.sample(selected_train_drugs, 40)
        # remaining_drugs_after_high = [drug for drug in selected_train_drugs if drug not in high_drugs]
        # low_drugs = random.sample(remaining_drugs_after_high, min(10, len(remaining_drugs_after_high)))
        # remaining_drugs = [drug for drug in remaining_drugs_after_high if drug not in low_drugs]
        # self.train_set = []
        # self.test_set = []
        # self.valid_set = []
        # drug_records_dict = defaultdict(list)
        # for record in self.data_list:
        #     drug_name = record[self.task]
        #     drug_records_dict[drug_name].append(record)
        # for drug in high_drugs:
        #     if drug in drug_records_dict:
        #         self.train_set.extend(drug_records_dict[drug][:100])
                # self.test_set.extend(drug_records_dict[drug][51:99])
        # for drug in low_drugs:
        #     if drug in drug_records_dict:
        #         self.test_set.extend(drug_records_dict[drug][:10])
        #         self.valid_set.extend(drug_records_dict[drug][11:100])
        #         self.test_set.extend(drug_records_dict[drug][101:201])
        self.used_pairs = []
    ...
    def get_batch_train_tasks(self, num_tasks=6):
        tasks = []
        batch_tasks = []
        task = []
        S_xd = []
        ...
        for _ in range(num_tasks):
            ...
            Q_xp_new = []
            Q_y_new = []
            # selected_drug = random.choice([d['protein_name'] for d in self.train_set])
            drug_candidates = [(d['protein_name'], d['drug_name']) for d in self.train_set]
            selected_tuple = random.choice(drug_candidates)
            selected_drug = random.choice(list(selected_tuple))
            drug_records = [rec for rec in self.data_list if rec.get('drug_name') == selected_drug or rec.get('protein_name') == selected_drug]
            split_point = int(len(drug_records) * 0.6)
            selected_S_xp_records = drug_records[:split_point]
            selected_Q_xp_records = drug_records[split_point:]
            # selected_S_xp_records = random.sample(drug_records, 20)
            for rec in selected_S_xp_records:
            ...
            remaining_drug_records = [rec for rec in drug_records if rec not in selected_S_xp_records]
            # selected_Q_xp_records = random.sample(remaining_drug_records, 15)
            # new_pairs = [(rec['drug_name'], rec['protein_name']) for rec in selected_S_xp_records + selected_Q_xp_records]
            # for pair in new_pairs:
            #     if pair not in self.used_pairs:
            #         self.used_pairs.append(pair)
            #     else:
            #         continue
            for rec in selected_Q_xp_records:
    ...
    def get_batch_valid_tasks(self, num_tasks=10):
        tasks = []
        batch_tasks = []
        task = []
        S_xd = []
        ...
        for _ in range(num_tasks):
            ...
            Q_xp_new = []
            Q_y_new = []
            # selected_drug = random.choice([d[self.task] for d in self.train_set])
            # drug_records = [rec for rec in self.data_list if rec[self.task] == selected_drug]
            # selected_S_xp_records = random.sample(drug_records, 20)
            drug_candidates = [(d['protein_name'], d['drug_name']) for d in self.train_set]
            selected_tuple = random.choice(drug_candidates)
            selected_drug = random.choice(list(selected_tuple))
            drug_records = [rec for rec in self.data_list if
                            rec.get('drug_name') == selected_drug or rec.get('protein_name') == selected_drug]
            split_point = int(len(drug_records) * 0.5)
            selected_S_xp_records = drug_records[:split_point]
            selected_Q_xp_records = drug_records[split_point:]
            for rec in selected_S_xp_records:
                S_xd.append(rec['drug'])
            ...
            # remaining_drug_records = [rec for rec in drug_records if rec not in selected_S_xp_records]
            # selected_Q_xp_records = random.sample(remaining_drug_records, 15)
            # new_pairs = [(rec['drug_name'], rec['protein_name']) for rec in
            #              selected_S_xp_records + selected_Q_xp_records]
            # for pair in new_pairs:
            #     if pair not in self.used_pairs:
            #         self.used_pairs.append(pair)
            #     else:
    ...
    def get_batch_test_tasks(self, num_test_tasks=2):
        tasks = []
        batch_tasks = []
        task = []
        S_xd = []
        ...
        for _ in range(num_test_tasks):
            ...
            # remaining_drug_records = [rec for rec in drug_records if rec not in selected_S_xp_records]
            # sample_size = 20
            # if len(remaining_drug_records) < sample_size:
            #     sample_size = len(remaining_drug_records)
            # selected_Q_xp_records = random.sample(remaining_drug_records, sample_size)
            for rec in selected_Q_xp_records:
                Q_xd.append(rec['drug'])
                Q_xp.append(rec['protein'])
                Q_y.append(rec['y'])
